...
//Rescue - imported files
	...
	const int timeRescueSimpleSide = 2200;
	...
	void SearchTriangle (byte sensor, bool alreadyInActuator = false) {
		...
		if ((ultra(sensor) < 265 && !actuator.hasVictim()) || alreadyInActuator) {
			if (((side_triangle == 'R' && sensor == 3) || (side_triangle == 'L' && sensor == 2)) || ((side_triangle == 'L' && sensor == 3 && (!DeadVictimReserved || time.millis() - timeToFind > timeRescueSimpleSide)) || (side_triangle == 'R' && sensor == 2 && (!DeadVictimReserved || time.millis() - timeToFind > timeRescueSimpleSide)))) {
				stop();
				console_led(2, $"$>Vítima<$ detectada a $>{(int)ultra(sensor)}<$ zm", color["cyan"]);
